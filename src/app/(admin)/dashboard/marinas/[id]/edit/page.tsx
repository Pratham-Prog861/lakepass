...
const editMarinaSchema = z.object({
...
type FormValues = z.infer<typeof editMarinaSchema>;
...
export default function EditMarinaPage() {
  ...
  const { data, isLoading, isError } = useQuery<{ marina: Marina }>({
    queryKey: ["marina", id],
    queryFn: async () => {
      const res = await fetch(`/api/marinas/${id}`);
  ...
  const {
    ...
    resolver: zodResolver(editMarinaSchema),
    defaultValues: {
      name: "",
      description: "",
      latitude: 0,
  ...
  // Populate form values when query data is loaded
  useEffect(() => {
    if (data?.marina) {
      setValue("name", data.marina.name);
      setValue("description", data.marina.description || "");
      setValue("latitude", data.marina.latitude);
      setValue("longitude", data.marina.longitude);
    }
  }, [data, setValue]);
  ...
  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      if (!res.ok) {
        ...
        throw new Error(errorData.error || "Failed to update marina");
      }
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["marina", id] });
      toast.success("Marina updated successfully!");
      router.push("/dashboard/marinas");
    },
    onError: (err: Error) => {
      toast.error(err.message || "Failed to update marina.");
    },
  });
  ...
  if (isError || !data?.marina) {
    return (
      <div className="rounded-2xl border border-red-100 bg-red-50/10 p-6 text-center text-xs font-semibold text-red-600 dark:border-red-950/20 dark:bg-red-950/10">
        Failed to load marina details or marina not found.
      </div>
    );
  }
    ...
    <div className="flex max-w-2xl flex-col gap-6">
      <div className="flex items-center gap-3">
        ...
        <div className="flex flex-col gap-0.5">
          ...
          <p className="text-xs text-zinc-500">Update coordinates or details for your marina.</p>
        </div>
      </div>
